# ShadowSpeak Low-Level Design — Backend
...
> This document covers the **backend** (Python FastAPI on AWS Lambda) portion of the LLD. See [04-Low-Level-Design-Mobile.md](04-Low-Level-Design-Mobile.md) for the mobile client portion and [06-Database-Design-Document.md](06-Database-Design-Document.md) for DynamoDB design.
...
- Offline sync queue design and conflict resolution (server side)
- Security implementation details, logging, and testing strategy
- Sequence diagrams for server-side core flows

### Out of Scope

- AWS infrastructure provisioning and deployment automation
- CI/CD pipeline implementation
- Mobile client state management, UI, native modules, and local storage
...
- one Amazon DynamoDB table (see [Database Design Document](06-Database-Design-Document.md))
- one local encrypted offline store on the client
...
- See the [Database Design Document](06-Database-Design-Document.md) for full key schemas.
...
- Query lessons by `level` and `topic` using `GSI2_LessonCatalog`.
- Get lesson detail by `lessonId` on the base table.
...
- User progress by `userId` with sort key for history.
...
## 4. Offline Sync Design (Server Side)
...
sequenceDiagram
    ...
    Api-->>App: sync result
    App->>App: mark queue items synced or retry
```
...
### 4.3 Retry Logic (Server Guidance)

- The client retries failed syncs with exponential backoff.
- The server should accept retries idempotently.
- Cap retries to avoid battery drain (client-side concern).
...
### 7.2 Browse → Download → Practice Flow
...
sequenceDiagram
    ...
    Api-->>App: result
    App->>App: update queue status
```

### 7.4 Ad Serving at Session Boundary
...
| LLD Component            | HLD Component                   | Functional Requirements | Use Cases                  |
| ------------------------ | -------------------------------- | ----------------------- | -------------------------- |
| Auth / Profile / Consent | Auth / Profile / Consent Module  | FR-1, FR-8, FR-9        | UC-01, UC-10, UC-11        |
| Content / Downloads      | Content / Downloads Module       | FR-2, FR-7              | UC-02, UC-06               |
| Session / Progress       | Session / Progress Module        | FR-3, FR-4, FR-5        | UC-03, UC-04, UC-05, UC-08 |
| Offline sync             | Session / Progress Module        | FR-3, FR-4, FR-5, FR-7  | UC-03, UC-04, UC-06        |
| Account Deletion         | Auth / Profile / Consent Module  | FR-8                    | UC-10                      |

## 10. Revision History

| Version | Date       | Author            | Description                                                 |
| ------- | ---------- | ----------------- | ----------------------------------------------------------- |
| 1.10    | 2026-05-14 | Backend Developer | Initial LLD draft for MVP modular backend — extracted from unified LLD |
