...
| FR-2 | **Lesson Catalog Browsing** -- Display a paginated list of available lessons with filters (level, topic, duration). Each lesson shows a topic thumbnail that is cached locally in app data. | User is authenticated.                                                                                                    | List of lessons displayed according to filter criteria, with thumbnails shown per topic.     | At least 20 lessons per page; filters return correct subset; topic thumbnails display from local cache and fall back to CDN download when missing. | Only free lessons are shown in MVP. Thumbnails are served via CDN URL from the Lesson schema and cached by topic key in app data. | Show message "No lessons found" when filter yields empty result. Show placeholder thumbnail when download fails.                                          |
